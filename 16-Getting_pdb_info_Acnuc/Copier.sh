...
cat ${DATA_PATH}${ORDER}/${FAMILY}/12-PDB_filter/Pdb_info.csv | while read LINE; do
    if [[ "${LINE}" = "Gene_family"* ]]; then
        echo ${LINE} >> ${DATA_PATH}${ORDER}/${FAMILY}/12-PDB_filter/temp.csv
        continue
    else
...
mv ${DATA_PATH}${ORDER}/${FAMILY}/12-PDB_filter/temp.csv ${DATA_PATH}${ORDER}/${FAMILY}/12-PDB_filter/Pdb_info.csv


# If all the pdb information was removed from the csv file, then we remove the directory
NB_LINES=$(wc -l ${DATA_PATH}${ORDER}/${FAMILY}/12-PDB_filter/Pdb_info.csv | cut -d' ' -f1)
