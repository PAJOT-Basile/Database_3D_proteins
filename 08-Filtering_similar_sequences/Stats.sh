...
cat $LIST_ORDERS | while read ORDER; do
    ...
    ls $DATA_PATH$ORDER | wc -l >> ./Stats/Number_families.txt
    ...
    for family in $LIST_FAMILIES; do
        if [[ $family = "Example" ]]; then
            continue
        elif [ -f "$DATA_PATH$ORDER/$family/04-Similar_sequences_removed/$family.fasta" ]; then
            n_seq=$(grep -c "^>" $DATA_PATH$ORDER/$family/04-Similar_sequences_removed/$family.fasta)
            ((nb_seq+=$n_seq))
            echo $n_seq >> ./Stats/${ORDER}_number_sequences_per_family.txt
            echo $family >> ./Stats/${ORDER}_names_of_families.txt
        fi
    done
    # Count the number of sequences per Super-Kingdom
    echo $nb_seq >> ./Stats/Number_sequences.txt
